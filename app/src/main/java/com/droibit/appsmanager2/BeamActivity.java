...
public class BeamActivity extends ListActivity {
	...
	private void processIntent(Intent intent) {
		...
		if (ndfRecords.length == 1) {
            final String line = new String(ndfRecords[0].getPayload()).substring(3);
            Debug.log("single : %s", line);
			IntentHepler.sendGooglePlay(this, new ListItem(line.split(",")).packageName);
			finish();
            return;
		}
        ...
        if  (!adapter.isEmpty()) {
            // ストアから戻った際に再び#processIntent(Intent)が走るので、リストが重複して作成される対策
            adapter.clear();
        }
